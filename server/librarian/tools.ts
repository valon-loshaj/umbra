...
import { Tool, ToolResultContent } from '../llm/types';
...
export const LIBRARIAN_TOOLS: Tool[] = [
	{
		name: 'semantic_search',
		description:
			'Search for notes semantically related to a query. Returns notes with similar meaning, not just matching text. Use this to find conceptually related content.',
		input_schema: {
			type: 'object',
			properties: {
				query: {
	...
	},
	{
		name: 'lexical_search',
		description:
			'Search for exact text matches in notes. Use this to find specific terms, names, or phrases.',
		input_schema: {
			type: 'object',
			properties: {
				query: {
	...
	},
	{
		name: 'read_file',
		description:
			'Read the content of a note. Use this to get the full text of a specific file.',
		input_schema: {
			type: 'object',
			properties: {
				path: {
	...
	},
	{
		name: 'list_structure',
		description:
			'List the folder structure of the vault or a specific folder. Shows files and subfolders.',
		input_schema: {
			type: 'object',
			properties: {
				folder: {
	...
	},
	{
		...
		description:
			'Get metadata about a note including frontmatter, file stats, and links. Does not return full content.',
		input_schema: {
			type: 'object',
			properties: {
				path: {
...
const toolExecutors: Record<string, ToolExecutor> = {
	async semantic_search(input, context): Promise<ToolResult> {
	...
	async lexical_search(input, context): Promise<ToolResult> {
		const query = input.query as string;
	...
	async read_file(input, context): Promise<ToolResult> {
	...
	async list_structure(input, context): Promise<ToolResult> {
	...
	async get_note_metadata(input, context): Promise<ToolResult> {
		const relativePath = input.path as string;
		const absolutePath = toAbsolutePath(relativePath, context.vaultPath);
...
function toAbsolutePath(relativePath: string, vaultPath: string): string {
